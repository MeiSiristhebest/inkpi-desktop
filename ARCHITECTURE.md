...
Proposal Ledger 以 IndexedDB 为 Desktop authoritative store；`RemoteProposalStore` 只把状态投影到 Daemon `proposal.sync.*` RPC，并携带 expectedRevision/stateHash。Selection Toolbar 已接入文本提案审阅路径；`proposalStateEvents` 提供带 workspace/project scope 的跨窗口通知，但跨设备提交和完整冲突 UI 仍待验证。
...
1. 五个 Slice 的完整真实 Desktop ↔ Daemon 集成链路（五个 task factory 的 child-process RPC、精确 output/status contract、磁盘 SQLite Daemon 进程重启后的任务状态/Artifact/Proposal 读取和 Continue 的 GhostText→Proposal/CAS headless 链路已有证据；章节保存→scheduler→diagnostic store→TipTap marker 的 Desktop 边界以及长任务取消/`waiting-user` UI 已有聚焦证据，但真实 provider、App restart 和完整生产验收仍未关闭）。
