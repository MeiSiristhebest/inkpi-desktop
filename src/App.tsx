...
export const App: React.FC = () => {
  ...
  if (mode === 'write') {
    return <Engine projectId={PROJECT_ID} onBack={() => setMode('doc')} />
  }
  ...
  return (
    <div className="h-screen w-screen flex overflow-hidden bg-[var(--ink-bg)] text-[var(--ink-text)] transition-colors duration-300">
      ...
      {aiPanelOpen && !isZenMode && (
        <aside className="w-[300px] shrink-0 flex flex-col border-l border-[var(--ink-border)] bg-[var(--ink-bg-panel)]">
          <div className="h-11 shrink-0 flex items-center justify-between px-3 border-b border-[var(--ink-border)]">
            ...
            <button onClick={() => setAiPanelOpen(false)} title="收起" className={iconBtn}>
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
          ...
          <div className="shrink-0 p-2.5 border-t border-[var(--ink-border)]">
            <div className="flex items-end gap-1.5">
              <input
                ...
                placeholder={isConnected ? '向 InkPi 下达写作指令…' : '离线模式：无法调用 AI'}
                disabled={!isConnected || aiBusy}
              ...
              />
              <button
                onClick={() => handleAiPrompt(aiInput)}
                disabled={!isConnected || aiBusy || !aiInput.trim()}
