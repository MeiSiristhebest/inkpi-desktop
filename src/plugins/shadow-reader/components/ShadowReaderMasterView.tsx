...
import { MessageSquare, Flame, ThumbsUp, AlertTriangle, ShieldAlert, Sparkles, Send } from "lucide-react"
import { clock } from "../../../adapters/clock"
import { idGenerator } from "../../../adapters/idGenerator"

export const ShadowReaderMasterView: FC<DesktopPluginViewProps> = ({ projectId, onStats }) => {
  const [chapterText, setChapterText] = useState(
    "林凡深吸一口气，退一步海阔天空，心想还是原谅了他吧，算了吧。\n然而下一刻，神秘黑衣人嘴角勾起一抹神秘微笑，目光微闪。\n林凡猛然暴起，一剑封喉！全场寂静，众人倒吸一口凉气！"
  )
  const [activeChapterId, setActiveChapterId] = useState("ch_01")
  ...
  const simulation: ShadowSimulationResult = ShadowReaderEngine.simulate(chapterText, activeChapterId)
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto text-slate-800 dark:text-slate-100">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl border bg-rose-50/50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900/40 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs text-rose-600 dark:text-rose-400 font-medium">暴怒/弃书毒点</div>
            <div className="text-xl font-bold text-rose-700 dark:text-rose-300">{simulation.sentimentSummary.rage} 条</div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/40 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">高潮喝彩/爽感</div>
            <div className="text-xl font-bold text-emerald-700 dark:text-emerald-300">{simulation.sentimentSummary.applause} 条</div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border bg-amber-50/50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/40 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs text-amber-600 dark:text-amber-400 font-medium">逻辑疑虑/伏笔</div>
            <div className="text-xl font-bold text-amber-700 dark:text-amber-300">{simulation.sentimentSummary.suspicious} 条</div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border bg-purple-50/50 dark:bg-purple-950/20 border-purple-200 dark:border-purple-900/40 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs text-purple-600 dark:text-purple-400 font-medium">期待/兴奋激动</div>
            <div className="text-xl font-bold text-purple-700 dark:text-purple-300">{simulation.sentimentSummary.excited} 条</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="border rounded-xl p-5 bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 space-y-3">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">章节正文推演区:</label>
          <textarea
        ...
        <div className="border rounded-xl p-5 bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 flex flex-col">
          ...
          <div className="space-y-3 mt-4 overflow-y-auto max-h-[300px] pr-1">
            {simulation.danmakus.length === 0 ? (
              <div className="text-xs text-slate-400 py-10 text-center">暂未捕捉到高辨识度读者情绪反应，正文叙事平稳。</div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold flex items-center gap-1.5">
                      ...
                      <span className="text-slate-400 text-[10px]">第 {d.paragraphIndex + 1} 段</span>
                    </span>
                    {d.isToxicAlert && (
