...
export const GoldChaptersMasterView: FC<DesktopPluginViewProps> = ({ projectId, onStats }) => {
  const [chaptersText, setChaptersText] = useState("林凡浑身浴血，冷冷看着面前逼迫退婚的赵家长老。他心中的复仇火焰熊熊燃烧。生死关头，识海中突然响起一声清脆提示：神级万古修罗系统已激活！今日你们给予的欺凌，来日誓要千百倍奉还！")
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto text-slate-800 dark:text-slate-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="border rounded-xl p-5 bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 space-y-3">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">前三章开篇文本输入:</label>
          <textarea
        ...
        <div className="border rounded-xl p-5 bg-slate-900 text-slate-100 border-amber-900/60 space-y-4 shadow-xl">
          ...
          <div className="text-center py-2 border-b border-slate-800">
            ...
            <div className={`text-4xl font-black mt-1 ${
              currentEval.score >= 80 ? "text-emerald-400" : currentEval.score >= 70 ? "text-amber-400" : "text-rose-400"
            }`}>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            <div className="p-2 rounded bg-slate-800 border border-slate-700">
              ...
              <div className="font-bold text-amber-400 mt-0.5">{currentEval.goldFingerScore} 分</div>
            </div>
            <div className="p-2 rounded bg-slate-800 border border-slate-700">
            ...
            <div className="p-2 rounded bg-slate-800 border border-slate-700">
              ...
              <div className="font-bold text-emerald-400 mt-0.5">{currentEval.expectationScore} 分</div>
            </div>
          </div>
      ...
      {evaluations.length > 0 && (
        <div className="border-t pt-4 border-slate-200 dark:border-slate-800">
          <div className="text-xs font-bold text-slate-400 mb-2">历史评测快照 ({evaluations.length})</div>
          <div className="flex gap-3 overflow-x-auto pb-2">
            {evaluations.map((e) => (
              <div key={e.id} className="p-3 bg-slate-100 dark:bg-slate-800 rounded min-w-[140px] text-xs space-y-1">
