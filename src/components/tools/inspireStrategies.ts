...
const OVERRIDES: Partial<Record<InspireCategory, Partial<Record<InspireStyle, Strategy>>>> = {
  ...
  门派: {
    西幻: () =>
      pickOne(['圣光', '黑铁', '狮心', '烈焰', '暗影']) +
      pickOne(['骑士团', '兄弟会', '圣殿', '评议会']),
    科幻: () =>
      pickOne(['星环', '天枢', '深空', '量子']) +
      pickOne(['联合舰队', '拓殖集团', '执政总署', '防卫军']),
  },
  功法: {
    科幻: () =>
      pickOne(['量子', '引力', '暗能量', '次声', '光棱']) +
      pickOne(['共振场', '超载矩阵', '偏转战法', '聚焦脉冲']),
  },
  地名: {
    科幻: () =>
      pickOne(['开普勒', '天狼', '半人马', '比邻']) +
      pickOne(['空间站', '轨道城', '前进基地', '中继星区']),
  },
  法宝: {
    科幻: () =>
      pickOne(['深空', '量子', '星火', '黎明']) +
      pickOne(['高频振荡刃', '磁轨步枪', '便携力场仪', '曲速驱动核心']),
  },
  怪物: {
    科幻: () =>
      pickOne(['异种', '基因变异', '深空潜伏', '虚空寄宿']) +
      pickOne(['猎手', '吞噬者', '主母', '母虫']),
  },
}
