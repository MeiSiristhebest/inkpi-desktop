...
export function useChapterEditorModel(args: UseChapterEditorModelArgs): ChapterEditorModel {
  ...
  const activateChapter = useCallback(
    async (
      ...
      if (!options?.skipDrain && autosave.hasPending()) {
        try {
          await autosave.drain()
        } catch (err) {
          reportSaveError(err)
  ...
  const deleteChapter = useCallback(
    async (chapter: ChapterRecord) => {
      const isCurrent = stateRef.current.activeChapterId === chapter.id
      if (isCurrent) {
        // 用户明确删除当前章节，意图是废弃，杜绝残留的待保存任务尝试存盘已删除章节
        autosave.cancel('chapter-deleted')
        draftJournal.clear(projectId, chapter.id)
      }
      ...
      if (isCurrent) {
        await activateChapter(nextList.length > 0 ? nextList[0] : null, { skipDrain: true })
      }
    },
    [patch, runPersistence, activateChapter, autosave, projectId],
  )
  ...
  // 监听持久化章节保存事件（如 AI 提案采纳、历史恢复、跨组件 mutation），保持模型权威 revision 同步
  useEffect(() => {
    return chapterSaveEvents.subscribe(({ chapter }) => {
      if (chapter.projectId === projectId) {
        const isCurrent = activeChapterRef.current?.id === chapter.id
        if (isCurrent) {
          activeChapterRef.current = chapter
        }
        patch({
          ...(isCurrent ? { activeChapter: chapter, isSaved: true } : {}),
          chapters: stateRef.current.chapters.map((c) => (c.id === chapter.id ? chapter : c)),
        })
      }
    })
  }, [projectId, patch])
