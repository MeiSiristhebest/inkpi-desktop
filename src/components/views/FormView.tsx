...
export const FormView: React.FC<FormViewProps> = ({ projectId, tabId, tabMeta, repository }) => {
  const { formData, isSaved, updateField, save } = useFormViewModel({
    projectId,
    tabId,
    repository,
  })
      ...
      <div>
        ...
        <div className="mt-4 max-h-[calc(100vh-180px)] overflow-y-auto space-y-6 pr-2">
          {modules.map((mod: any, idx: number) => (
            <div
              key={idx}
              className="bg-[var(--ink-bg-card)] border border-[var(--ink-border)] rounded-xl p-5 shadow-xs"
            >
