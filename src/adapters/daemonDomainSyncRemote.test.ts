...
describe('daemon domain sync adapter', () => {
  it('maps the four projection operations to the daemon RPC contract', async () => {
    const request = vi.fn(async (method: string) => {
      if (method === 'domain.sync.push')
        return { accepted: true, duplicate: false, workspaceId: 'project-1', revision: 1 }
      if (method === 'domain.sync.pull') return [changeSet]
      if (method === 'domain.sync.snapshot')
        return { workspaceId: 'project-1', revision: 1, changeSets: [changeSet], createdAt: 1 }
