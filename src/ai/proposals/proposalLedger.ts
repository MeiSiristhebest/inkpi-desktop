...
export class ProposalConflictError extends Error {
  constructor(proposalId: string, expected: number, actual: number, message?: string) {
    super(
      message ??
        `Proposal ${proposalId} is stale: expected revision ${expected}, received ${actual}`,
    )
...
export function validateProposal(proposal: AiProposal): void {
  ...
  if (proposal.updatedAt !== undefined)
    assertProposalTimestamp(proposal.updatedAt, 'Proposal updatedAt')
  ...
  if (proposal.inversePatches !== undefined) {
    if (!Array.isArray(proposal.inversePatches))
      throw new Error('Proposal inverse patches are invalid')
