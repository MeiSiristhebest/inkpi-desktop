...
export class RemoteProposalStore implements AtomicProposalStore {
  ...
  async compareAndSwapProposal(expected: AiProposal, next: AiProposal): Promise<boolean> {
    if (
      'compareAndSwapProposal' in this.local &&
      typeof (this.local as any).compareAndSwapProposal === 'function'
    ) {
  ...
  async acquireProposalOperation(
    ...
    if (
      'acquireProposalOperation' in this.local &&
      typeof (this.local as any).acquireProposalOperation === 'function'
    ) {
  ...
  async completeProposalOperation(
    claim: ProposalOperationClaim,
    next: AiProposal,
  ): Promise<boolean> {
    if (
      'completeProposalOperation' in this.local &&
      typeof (this.local as any).completeProposalOperation === 'function'
    ) {
  ...
  async releaseProposalOperation(claim: ProposalOperationClaim): Promise<void> {
    if (
      'releaseProposalOperation' in this.local &&
      typeof (this.local as any).releaseProposalOperation === 'function'
    ) {
